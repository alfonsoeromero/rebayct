...
public class MNaiveBayes {
    ...
    private void usage(){
        ...
        System.err.println(" -apriori=FILENAME : name of the file with apriori results (OPTIONAL PARAMETER)");
        System.exit(-1);
    }
    
    public void processArguments(String args[]) {
        if (args.length < 3 || args.length > 4) this.usage();
        ...
        for (String arg : args) {
            if (arg.trim().startsWith("-output=")) {
                ...
                usedArgs[2] = true;
            } else if (arg.trim().startsWith("-apriori=")) {
                this.aprioriFileName = arg.trim().substring(9).trim();
                File route = new File(aprioriFileName);
                if (!route.exists()) {
                    System.err.println("ERROR: classified file " + aprioriFileName+ " does not exist. Exiting.");
                    System.exit(-1);
                }
            } else {
    ...
    public static void main(String args[]) {
        MNaiveBayes mn = new MNaiveBayes();
        
        // arguments processing
        mn.processArguments(args);
        
        // class initialization
        MultinomialNaiveBayes naive = new MultinomialNaiveBayes(mn.classIndexFileName);
        
        // we set the file of aprioris, if any
        if (mn.aprioriFileName != null)
            naive.setAPrioriFile(mn.aprioriFileName);
        ...
        try {
            ...
            while (in.ready()) {
                ...
                List<Map.Entry<Integer, Double>> sortedList = new ArrayList<Map.Entry<Integer, Double>>(list.entrySet());
                
                // Sort the list using an annonymous inner class implementing Comparator for the compare method
                java.util.Collections.sort(sortedList, new Comparator<Map.Entry<Integer, Double>>(){
